[![npm version](https://badge.fury.io/js/@omegion1npm/quidem-cumque-cumque.svg)](http://badge.fury.io/js/@omegion1npm/quidem-cumque-cumque)
[![Node.js CI](https://github.com/omegion1npm/quidem-cumque-cumque/actions/workflows/node.js.yml/badge.svg)](https://github.com/omegion1npm/quidem-cumque-cumque/actions/workflows/node.js.yml)
[![Windows Tests](https://img.shields.io/appveyor/ci/tulios/@omegion1npm/quidem-cumque-cumque.svg?label=Windows%20Tests)](https://ci.appveyor.com/project/tulios/@omegion1npm/quidem-cumque-cumque)
# Mappersmith
...
- [Usage](#usage)
  ...
  - [Testing Mappersmith](#testing-@omegion1npm/quidem-cumque-cumque)
  - [Gateways](#gateways)
...
```sh
npm install @omegion1npm/quidem-cumque-cumque --save
```

or

```sh
yarn add @omegion1npm/quidem-cumque-cumque
```
...
```typescript
import forge, { configs } from "@omegion1npm/quidem-cumque-cumque"
import { Fetch } from "@omegion1npm/quidem-cumque-cumque/gateway/fetch"
...
```javascript
const forge = require("@omegion1npm/quidem-cumque-cumque").default;
const { configs } = require("@omegion1npm/quidem-cumque-cumque");
const FetchGateway = require("@omegion1npm/quidem-cumque-cumque/gateway/fetch").default;
```
...
import { configs } from '@omegion1npm/quidem-cumque-cumque'
...
* `request()` - Returns the original [Request](https://github.com/omegion1npm/quidem-cumque-cumque/blob/master/src/request.js)
...
The `response` method receives a function which returns a `Promise` resolving the [Response](https://github.com/omegion1npm/quidem-cumque-cumque/blob/master/src/response.js). This function must return a `Promise` resolving the Response. The method `enhance` can be used to generate a new response based on the previous one.
...
Why is it not safe? Basically, the setContext function mutates a global state (see [here](https://github.com/omegion1npm/quidem-cumque-cumque/blob/2.34.0/src/@omegion1npm/quidem-cumque-cumque.js#L114)), hence it is the last call to setContext that decides its global value. Which leads to a race condition when handling concurrent requests.
...
"renew" can only be invoked sometimes before it's considered an infinite loop, make sure your middleware can distinguish an error from a "renew". By default, @omegion1npm/quidem-cumque-cumque will allow 2 calls to "renew". This can be configured with `configs.maxMiddlewareStackExecutionAllowed`. It's advised to keep this number low. Example:
...
If an infinite loop is detected, @omegion1npm/quidem-cumque-cumque will throw an error.
...
```javascript
import { BasicAuthMiddleware } from '@omegion1npm/quidem-cumque-cumque/middleware'
...
```javascript
import { CsrfMiddleware } from '@omegion1npm/quidem-cumque-cumque/middleware'
...
```javascript
import { DurationMiddleware } from '@omegion1npm/quidem-cumque-cumque/middleware'
...
```javascript
import { EncodeJsonMiddleware } from '@omegion1npm/quidem-cumque-cumque/middleware'
...
```javascript
import { GlobalErrorHandlerMiddleware, setErrorHandler } from '@omegion1npm/quidem-cumque-cumque/middleware'
...
```javascript
import { LogMiddleware } from '@omegion1npm/quidem-cumque-cumque/middleware'
...
```javascript
import { RetryMiddleware } from '@omegion1npm/quidem-cumque-cumque/middleware'
...
```javascript
import { TimeoutMiddleware } from '@omegion1npm/quidem-cumque-cumque/middleware'
...
This section is only relevant for @omegion1npm/quidem-cumque-cumque versions older than but not including `2.27.0`, when the method `prepareRequest` did not exist. This section describes how to create a middleware using older versions.

Since version `2.27.0` a new method was introduced: `prepareRequest`. This method aims to replace the `request` method in future versions of @omegion1npm/quidem-cumque-cumque, it has a similar signature as the `response` method and it is always async. All previous middleware are backward compatible, the default implementation of `prepareRequest` will call the `request` method if it exists.

The `request` method receives an instance of the [Request](https://github.com/omegion1npm/quidem-cumque-cumque/blob/master/src/request.js) object and it must return a Request. The method `enhance` can be used to generate a new request based on the previous one.
...
## <a name="testing-@omegion1npm/quidem-cumque-cumque"></a> Testing Mappersmith
...
```javascript
import { install, uninstall } from '@omegion1npm/quidem-cumque-cumque/test'
...
```javascript
import { forge } from '@omegion1npm/quidem-cumque-cumque'
import { install, uninstall, mockClient } from '@omegion1npm/quidem-cumque-cumque/test'
...
```javascript
import { m } from '@omegion1npm/quidem-cumque-cumque/test'
...
```javascript
import { forge } from '@omegion1npm/quidem-cumque-cumque'
import { install, uninstall, mockRequest } from '@omegion1npm/quidem-cumque-cumque/test'
...
```javascript
import { install, uninstall, unusedMocks } from '@omegion1npm/quidem-cumque-cumque/test'
...
You can write your own gateway, take a look at [XHR](https://github.com/omegion1npm/quidem-cumque-cumque/blob/master/src/gateway/xhr.js) for an example. To configure, import the `configs` object and assign the gateway option, like:
...
The new configurations will be merged. `configure` also receives the `requestParams` as the first argument. Take a look [here](https://github.com/omegion1npm/quidem-cumque-cumque/blob/master/src/@omegion1npm/quidem-cumque-cumque.js) for more options.
...
Take a look [here](https://github.com/omegion1npm/quidem-cumque-cumque/blob/master/src/@omegion1npm/quidem-cumque-cumque.js) for more options.
...
```javascript
import { FetchGateway } from '@omegion1npm/quidem-cumque-cumque/gateway'
import { configs } from '@omegion1npm/quidem-cumque-cumque'
...
```typescript
import type { Middleware } from '@omegion1npm/quidem-cumque-cumque'
...
```typescript
import { forge } from '@omegion1npm/quidem-cumque-cumque'
import { mockClient } from '@omegion1npm/quidem-cumque-cumque/test'
...
In remote project:
```sh
yarn link ../@omegion1npm/quidem-cumque-cumque/dist
```
...
https://github.com/omegion1npm/quidem-cumque-cumque/graphs/contributors

## License

See [LICENSE](https://github.com/omegion1npm/quidem-cumque-cumque/blob/master/LICENSE) for more details.
